...
func TestOne(t *testing.T) {
	f := func(in string) int {
		sum := 0
		for _, row := range strings.Split(in, "\n") {
			cells := strings.Split(row, "	")
			v, _ := strconv.Atoi(cells[0])
			min := v
			max := v
			for _, c := range cells {
				v, _ := strconv.Atoi(c)
...
2	4	6	8`, 18},
	...
	for _, c := range cc {
		allPassed = t.Run(c.in, func(t *testing.T) {
			if f(c.in) != c.out {
				t.Errorf("expected %d, got %d", c.out, f(c.in))
			}
		}) && allPassed
	}
	if !allPassed {
		t.Fatal("at least one testcase didn't succeed")
	}
	fmt.Println("One:", f(input))
}
...
func TestTwo(t *testing.T) {
	f := func(in string) int {
		sum := 0
		for i, c := range in {
			if byte(c) == in[(i+len(in)/2)%len(in)] {
				sum += int(c - '0')
			}
		}
		return sum
