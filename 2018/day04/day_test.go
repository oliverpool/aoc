...
func (d date) time() time.Time {
	return time.Date(d.year, time.Month(d.month), d.day, d.hour, d.minute, 0, 0, time.Local)
}

func (d date) midnightAfter() time.Time {
	t := d.time()
	if d.hour < 1 {
		return t
	}
	if d.minute == 0 {
		return t.Add(time.Duration(23-d.hour) * time.Hour)
	}
	return t.Add(time.Duration(22-d.hour) * time.Hour).Add(time.Duration(60-d.minute) * time.Minute)
}

func (d date) midnightBefore() time.Time {
	if d.hour > 1 {
		d.hour = 0
		d.minute = 59
	}
...
func (s shift) duration() int {
	start := s.start.midnightAfter()
	end := s.end.midnightBefore()
...
func TestOne(t *testing.T) {
	...
	var lastAwake date
	var shifts []shift
	var sleepShifts []shift
	for _, line := range strings.Split(input, "\n") {
		var d date
		var action string
		var _a string
		fmt.Sscanf(line, "[%d-%d-%d %d:%d] %s %s", &d.year, &d.month, &d.day, &d.hour, &d.minute, &_a, &action)
		if strings.HasPrefix(action, "#") {
			...
			currentGuard, _ = strconv.Atoi(action[1:])
			lastAwake = d
			...
			sleepShifts = append(sleepShifts, shift{
				guard: currentGuard,
				start: lastAwake,
				end:   d,
			})
	...
	cumSleeps := make(map[int]int)
	for _, s := range sleepShifts {
		cumSleeps[s.guard] += s.duration()
	}

	type guardMin struct {
		guard, minute int
	}
	minutes := make(map[guardMin]int)
	for _, s := range sleepShifts {

		current := s.start.midnightBefore()
		end := s.end.midnightAfter()
		if current.Sub(end) < -time.Hour {
			t.Fatal(current, end, "too long")
		}
		for current.Sub(end) < 0 {
			minutes[guardMin{s.guard, current.Minute()}] += 1
	...
	max := 0
	var maxGuardMin guardMin
	for min, value := range minutes {
		if value > max {
			max = value
			maxGuardMin = min
		}
	...
	t.Fatal(maxGuardMin, maxGuardMin.guard*maxGuardMin.minute)
}

var tTest = `[1518-11-01 00:00] Guard #10 begins shift
...
var input = `[1518-02-28 23:56] Guard #971 begins shift
